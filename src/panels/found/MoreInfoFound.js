...
class MoreInfoFound extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            activePanel: "moreInfoFound",
    ...
    render() {
        ...
        return (
            <UI.Panel id='moreInfoFound'>
                <UI.PanelHeader noShadow left={<UI.HeaderButton onClick={this.props.go} data-to={this.props.from === "myAds" ? "myAds" : "found"}>{<Icon24Back/>}</UI.HeaderButton>}>Подробности</UI.PanelHeader>
                {this.state.fetchInProgress ? <UI.ScreenSpinner/> :
                    <UI.Div>
                        ...
                        <UI.Group title="Информация">
                            ...
                            <UI.Cell multiline>
                                <UI.InfoRow title="Когда найдено:">
                        ...
                        <UI.Group title="Местоположение">
                            <UI.Cell multiline>
                                <UI.InfoRow title="Где найдено:">
                                    {this.state.thingInfo.place}
                                </UI.InfoRow>
                            </UI.Cell>
                            <UI.Div>
                                {this.state.thingInfo.lat !== "null" && <UI.Button style = {centerStyle} before={<Icon16Map/>} size = "l" onClick={() => {
